...
```
nmap <ipv4 or ipv6>
	> if <target.com> -> DNS -> <ip>s (--resolve-all or only scan one <ip>)
	> support <ip>/<numbits>
		> <numbits> = 0 target the whole Internet!
		> <numbits> = 32, 128 as max for IPv4, IPv6
	> support skipping some <ip>s such as
		> 192.168.0-255.1-254
		> 192.168.3-5,7.1
	> support <host1> <host2>
		> `nmap scanme.nmap.org 192.168.0.0/8 10.0.0,1,3-7.-`
	> use `-iL <inputfilename>`
